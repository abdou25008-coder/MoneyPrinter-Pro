...
MAX_SCRIPT_PARAGRAPH_NUMBER = 30
...
def extract_thematic_anchor(video_subject: str, video_script: str = "") -> str:
    """
    Extract a cohesive visual environment anchor in English based on the video subject.
    Ensures stock video searches remain strictly locked to the target physical universe
    (e.g., deep sea, space, forest, medical, ancient history), avoiding scattered clips.
    """
    combined_text = f"{video_subject} {video_script[:300]}".lower()

    # Marine / Deep Sea / Oceans / Underwater
    marine_keys = [
        "بحر", "بحار", "محيط", "محيطات", "أعماق", "حوت", "حيتان", "سمك", "أسماك",
        "مرجان", "مرجانية", "غوص", "غواصة", "سفينة غارقة", "ماريانا", "قاع البحر",
        "sea", "ocean", "underwater", "marine", "abyss", "deep sea", "coral",
        "shark", "submarine", "submersible", "trench", "aquatic"
    ]
    if any(k in combined_text for k in marine_keys):
        return "deep sea ocean underwater marine abyss"

    # Outer Space / Astronomy / Universe
    space_keys = [
        "فضاء", "كون", "مجرة", "مجرات", "كوكب", "كواكب", "نجوم", "نجم", "قمر",
        "شمس", "مركبة فضاء", "فلك", "نيزك", "ثقب أسود",
        "space", "universe", "galaxy", "cosmos", "planet", "planets", "astronaut",
        "solar system", "nebula", "black hole"
    ]
    if any(k in combined_text for k in space_keys):
        return "outer space galaxy cosmos planets universe"

    # Desert / Dunes
    desert_keys = [
        "صحراء", "رمال", "كثبان", "واحة", "جمال", "بادية",
        "desert", "sand dunes", "sahara", "arid landscape"
    ]
    if any(k in combined_text for k in desert_keys):
        return "desert sand dunes arid landscape"

    # Forest / Rainforest / Wildlife
    nature_keys = [
        "طبيعة", "غابة", "غابات", "أشجار", "حيوانات", "برية", "شلال", "شلالات", "جبال",
        "forest", "jungle", "rainforest", "wildlife", "nature", "waterfall", "trees"
    ]
    if any(k in combined_text for k in nature_keys):
        return "nature forest wildlife landscape"

    # Technology / AI / Robotics
    tech_keys = [
        "ذكاء اصطناعي", "روبوت", "روبوتات", "برمجة", "تكنولوجيا", "تقنية", "حاسوب", "سيبراني", "مستقبل",
        "ai", "artificial intelligence", "robot", "robotics", "coding", "cyber", "futuristic"
    ]
    if any(k in combined_text for k in tech_keys):
        return "modern technology artificial intelligence digital futuristic"

    # History / Ancient Civilizations
    history_keys = [
        "تاريخ", "حضارة", "حضارات", "فراعنة", "أهرامات", "آثار", "قديم", "مومياء", "قلعة", "معبد",
        "history", "ancient", "pyramid", "pyramids", "civilization", "pharaoh", "monument"
    ]
    if any(k in combined_text for k in history_keys):
        return "ancient history civilization archaeological heritage"

    # Medical / Healthcare
    medical_keys = [
        "طب", "صحة", "طبيب", "مستشفى", "خلايا", "فيروس", "دواء", "جراحة",
        "medical", "healthcare", "doctor", "hospital", "laboratory", "cells", "biology"
    ]
    if any(k in combined_text for k in medical_keys):
        return "medical healthcare laboratory scientific"

    # Business / Finance
    business_keys = [
        "مال", "تجارة", "اقتصاد", "استثمار", "بورصة", "عملة", "بنوك", "أموال",
        "business", "finance", "money", "stock market", "investment", "corporate"
    ]
    if any(k in combined_text for k in business_keys):
        return "business corporate financial investment office"

    return ""


def post_process_thematic_terms(
    terms: List[str],
    video_subject: str,
    thematic_anchor: str = "",
) -> List[str]:
    """
    Ensures every search term is strictly bound to the overarching physical theme,
    preventing abstract or irrelevant stock footage drift (InVideo style).
    """
    if not terms:
        return []

    anchor = (thematic_anchor or extract_thematic_anchor(video_subject)).strip()
    anchor_keywords = anchor.split() if anchor else []

    processed = []
    seen = set()

    for term in terms:
        t = str(term or "").strip().strip('"\'')
        if not t:
            continue

        # If a thematic anchor is detected, make sure the term isn't an isolated abstract word
        if anchor_keywords:
            term_lower = t.lower()
            has_anchor_token = any(k.lower() in term_lower for k in anchor_keywords[:3])
            
            # If the term lacks the visual setting or is only 1-2 generic words, anchor it
            if not has_anchor_token:
                # Prefix with key visual setting (first 2 words of the anchor)
                prefix = " ".join(anchor_keywords[:2])
                t = f"{prefix} {t}".strip()
    ...
    # If list is empty but anchor exists, provide high-quality default anchor queries
    if not processed and anchor:
        processed = [
            f"{anchor} cinematic 4k",
            f"{anchor} close up documentary",
            f"{anchor} wide landscape exploration",
        ]
...
def generate_terms(
    ...
    if thematic_anchor:
        anchor_instruction = (
            f"7. PRIMARY THEMATIC VISUAL ANCHOR: The overarching physical environment for this video is strictly "
            f"'{thematic_anchor}'. Every single search term MUST be situated directly within this physical setting. "
            f"Never output generic terms (e.g., instead of 'pressure', output '{thematic_anchor} water pressure')."
        )

    if match_script_order:
        goal = (
            f"Generate {amount} chronological, visually consistent stock-video search terms that follow "
            "the order of topics in the video script (InVideo Style)."
        )
        ordering_rule = (
        ...
        example_terms = [
            f"{thematic_anchor} opening visual scene" if thematic_anchor else "opening visual topic",
            *[
                f"{thematic_anchor} scene visual moment {index}" if thematic_anchor else f"script visual topic {index}"
                for index in range(2, max(amount, 1))
            ],
            f"{thematic_anchor} climactic final scene" if thematic_anchor else "final visual topic",
        ]
        output_example = json.dumps(example_terms[:amount], ensure_ascii=False)
    else:
        goal = (
            f"Generate {amount} cohesive, highly-relevant search terms for stock videos (Pexels, Pixabay, Coverr) "
            f"grounded in the subject of the video (InVideo Style)."
        )
        ordering_rule = ""
        output_example = (
            '["underwater deep sea ocean trench", "deep sea bioluminescent jellyfish underwater", '
            '"underwater submarine exploring abyss", "deep ocean floor hydrothermal vents", "deep sea creature abyss"]'
            if "deep sea" in thematic_anchor
            else '["search term 1", "search term 2", "search term 3", "search term 4", "search term 5"]'
        )

    prompt = f"""
# Role: InVideo-Style Thematic Stock Video Search Terms Generator

## Goals:
{goal}

## Constraints & Visual Cohesion Rules:
1. The search terms must be returned as a JSON array of strings.
2. Each search term must describe concrete, realistic B-roll footage (2-4 words).
3. You must ONLY return the JSON array of strings. Do not return markdown, explanations, or the script.
4. STRICT THEMATIC ANCHORING: Every term MUST be anchored in the video's physical environment. Never output standalone abstract words like "pressure", "darkness", "mystery", "silence", "danger", or "beginning".
...
Please note that you must use English for generating video search terms; Chinese or Arabic is not accepted.
""".strip()

    logger.info(
        f"subject: {video_subject}, match_script_order: {match_script_order}, "
        f"thematic_anchor: {thematic_anchor or '<auto>'}"
    )
    ...
    # Post-process terms through InVideo Thematic Grounding to guarantee absolute consistency
    search_terms = post_process_thematic_terms(
        search_terms, video_subject=video_subject, thematic_anchor=thematic_anchor
    )

    logger.success(f"completed thematic search terms: \n{search_terms}")
    return search_terms
